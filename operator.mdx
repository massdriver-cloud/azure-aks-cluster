...
Connecting to Grafana running on your AKS cluster requires setting up `kubectl` from above. After `kubectl` is set up, you can port forward the service locally using: `kubectl port-forward service/massdriver-grafana 3000:80 --namespace=md-observability` and then browsing to `http://localhost:3000`. The username is `admin` and the password is the password you set on the bundle configuration page.
